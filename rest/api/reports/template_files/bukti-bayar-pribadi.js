...
function parsingDataUnit(dataResponse) {
    let dataDistinctUnit = dataResponse.filter(
        (value, index, self) =>
            self.findIndex((y) => y.id_unit === value.id_unit) === index
    )
    let dataDistinctBlok = dataResponse.filter(
        (value, index, self) =>
            self.findIndex((y) => y.nama_blok === value.nama_blok) === index
    )
    let dataJenisUnit = dataResponse.filter(
        (value, index, self) =>
            ...
            dataDistinctBlok.length === 0
                ...
                : dataDistinctBlok.length === 1
                    ...
                    : dataDistinctBlok.length > 3
                        ? `${dataDistinctBlok[0].nama_blok}...${dataDistinctBlok[dataDistinctBlok.length - 1].nama_blok
                        }`
                        : dataDistinctBlok.length === 3
            ...
            dataDistinctUnit.length === 0
                ...
                : dataDistinctUnit.length === 1
                    ...
                    : dataDistinctUnit.length > 3
                        ? `${dataDistinctUnit[0].nama_unit}...${dataDistinctUnit[dataDistinctUnit.length - 1].nama_unit
                        }`
                        : dataDistinctUnit.length === 3
...
function parsingDataListrik(dataResponse, dataDefaultTarifRusun) {
    let indexDataListrik = dataResponse.findIndex(
    ...
    for (let i = 0; i < dataResponse.length; i++) {
        ...
        if (indexDspParsing >= 0) {
            ...
            namaDsp = dataMapDisplay[indexDspParsing].dsp_use_nama_default
                ...
                : dataResponse[i][
                    dataMapDisplay[indexDspParsing].dsp_nama_map
                ].toUpperCase()
...
module.exports = function (data) {
    ...
    try {
        for (var dd = 0; dd < data.length; dd++) {
            console.log(info[dd])
            var datUnit = data[dd].unit !== null && typeof (data[dd].unit) !== 'undefined' ? data[dd].unit : []
            var datDetil = data[dd].data_detil_invoice !== null && typeof (data[dd].data_detil_invoice) !== 'undefined' ? data[dd].data_detil_invoice : []
            var datMapDsp = data[dd].map_dsp !== null && typeof (data[dd].map_dsp !== 'undefined') ? data[dd].map_dsp : []
            var datListrik = data[dd].tarif_listrik_rusun_on_blth !== null && typeof (data[dd].tarif_listrik_rusun_on_blth) !== 'undefined' ? data[dd].tarif_listrik_rusun_on_blth : []
            var datAir = data[dd].tarif_air_rusun_on_blth !== null && typeof (data[dd].tarif_air_rusun_on_blth) !== 'undefined' ? data[dd].tarif_air_rusun_on_blth : []
            var rusun = data[dd].data_rusun !== null && typeof (data[dd].data_rusun !== 'undefined') ? data[dd].data_rusun : []
            var dataKontrak = parsingDataKontrak(data[dd])
            ...
            for (var i = 0; i < particular_rang.dataSum.length; i++) {
                if (i === 0) {
                    content_part.push(
                        [{ text: particular_rang.dataSum[i].nama, border: [true, true, false, false] }, { text: currency(particular_rang.dataSum[i].nominal), border: [true, true, true, false], alignment: 'right' }]
                    )
                } else {
                    content_part.push(
                        [{ text: particular_rang.dataSum[i].nama, border: [true, false, false, false] }, { text: currency(particular_rang.dataSum[i].nominal), border: [true, false, true, false], alignment: 'right' }]
                    )
                }

            }
            content_part.push(
                [{ text: 'GRAND TOTAL', border: [true, false, false, true], bold: true }, { text: 'Rp. ' + currency(particular_rang.dataSumTotal), border: [true, true, true, true], alignment: 'right', bold: true }]
            )
            ...
            for (let i = 0; i < lstrkair.dataListrikAir.length; i++) {
                content_lstrkair.push(
                    [
                        ...
                        lstrkair.dataListrikAir[i].meter.toUpperCase() == 'ELECTRICITY' ? currency(lstrkair.dataListrikAir[i].demand_charges) : '-',
                        lstrkair.dataListrikAir[i].meter_start !== null ? lstrkair.dataListrikAir[i].meter_start : 0,
                        lstrkair.dataListrikAir[i].meter_end !== null ? lstrkair.dataListrikAir[i].meter_end : 0,
                        lstrkair.dataListrikAir[i].meter_pemakaian,
                        lstrkair.dataListrikAir[i].nominal_pemakaian,
                        lstrkair.dataListrikAir[i].meter.toUpperCase() == 'ELECTRICITY' ? currency(lstrkair.dataListrikAir[i].nominal_pju) : '-',
                        lstrkair.dataListrikAir[i].meter.toUpperCase() == 'WATER' ? currency(lstrkair.dataListrikAir[i].nominal_wmm) : '-',
            ...
            content_pdf.push([
                {
                    ...
                    table: {
                        ...
                        body: [
                            ...
                            ['', { text: rusun[0].nama_rusun !== '' ? rusun[0].nama_rusun : '-', fontSize: 11, bold: true, alignment: 'center' }],
                            ['', { text: '' + (rusun[0].lokasi !== '' ? rusun[0].lokasi : '-') + ' - ' + (rusun[0].kecamatan !== '' ? rusun[0].kecamatan : '-') + ' - ' + rusun[0].provinsi + '', fontSize: 11, bold: true, alignment: 'center' }],
                            ['', { text: 'Telp. ' + (rusun[0].telpon !== '' ? rusun[0].telpon : '-') + ' Fax ' + (rusun[0].fax !== '' ? rusun[0].fax : '-') + '', fontSize: 10, italics: true, alignment: 'center' }],
                ...
                },
                {
                    ...
                    table: {
                        widths: [100, 20, 'auto'],
                        body: [
                            [{ text: 'Invoice No.', bold: true }, ':', data[dd].no_invoice],
                            [{ text: 'Receipt From', bold: true }, ':',
                            data[dd].jenis_registrasi === 'P'
                                ? (
                                    data[dd].pihak2_nama_perusahaan !== null
                                        ? data[dd].pihak2_nama_perusahaan
                                        : '-'
                                )
                                : (
                                    data[dd].pihak2_nama_lengkap !== null
                                        ? data[dd].pihak2_nama_lengkap
                                        : '-'
                                )],
                ...
                },
                {
                    ...
                    table: {
                        widths: [70, 215, 50],
                        body: [
                            ['Blok / Nomor', ': ' + unit.blok, unit.jmlhUnit],
                            ['Rate/KWH', ':  ' + (hitungLstrk.rate !== null ? currency(hitungLstrk.rate) : currency(0)), ''],
                            [' ', ' ', ' '],
                            ['Demand Charges', ':  ' + (hitungLstrk.faktorPengali !== null ? hitungLstrk.faktorPengali : 0) + ' x ' + (hitungLstrk.demandCharges !== null ? currency(hitungLstrk.demandCharges) : currency(0)), ''],
                            ['PJU', ':   ' + (hitungLstrk.pju !== null ? currency(hitungLstrk.pju) : currency(0)) + '%', ''],
                            ['Rate/M3', ':   ' + (hitungAir.rate !== null ? currency(hitungAir.rate) : currency(0)),
                                { text: 'Water Meter Maintenance (WMM) : ' + (hitungAir.wmm !== null ? hitungAir.wmm : 0), margin: [-105, 0, 0, 0] }
                            ],
                            [' ', ' ', ' '],
                            ['Tgl Jatuh tempo', ': ' + moment(data[dd].periode_bln_sewa_awal).format("DD MMMM YYYY"), ''],
                        ]
                    },
                    layout: 'noBorders'
                },
                ...
                },
                {
                    ...
                    table: {
                        body: [
                            ['Periode Sewa Kamar Dari Tanggal ', ':', moment(data[dd].periode_bln_sewa_awal).format("DD MMMM YYYY") + ' s/d ' + moment(data[dd].periode_bln_sewa_akhir).format("DD MMMM YYYY")],
                            ['Meter Listrik & Air Dicatat Mulai Tanggal ', ':', (dateListrik_start !== null ? moment(dateListrik_start).format("DD MMMM YYYY") : '-') + (dateListrik_end !== null ? ' - ' + (moment(dateListrik_end).format("DD MMMM YYYY")) : '')]
                ...
                },
                {
                    text: [
                        ...
                        'NB : ' + dataKontrak.say.toUpperCase()
                    ],
                    fontSize: 9,
                    bold: true,
                    italics: true,
                ...
                },
                {
                    ...
                    table: {
                        ...
                        heights: [25, 25],
                        body: [
                            ["Thank's For Your Payment's", { text: '' + rusun[0].provinsi + ', ' + moment(rusun[0].tgl_invoice).format('DD MMMM YYYY'), alignment: 'center' }],
                            [
                                {
                                    text: [
                                        { text: (info[dd].media_pembayaran === 'T' ? 'Transfer' : 'Cash') + ' Pada Tanggal : ' },
                                        { text: moment(info[dd].tgl_pembayaran).format('DD MMMM YYYY'), bold: true, border: [true, true, true, true] }
                                    ]
                                },
                                ''
                            ],
                            [{ text: currency(info[dd].nominal_pembayaran), bold: true }, { text: dataKontrak.ttdNama.toUpperCase(), alignment: 'center' }]
                        ]
