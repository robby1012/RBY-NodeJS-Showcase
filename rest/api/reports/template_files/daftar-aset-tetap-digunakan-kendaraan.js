const moment = require('moment')
const { isNumber } = require("validate.js");

function currency(curr) {
	var data;
	if (!isNumber(curr)) {
		data = (parseFloat(curr)).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
	} else if (isNumber(curr)) {
		data = (curr).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
	}

	//console.log(data.toString())
	var str = data.toString();

	var comma = str.split('.');
	var txt = comma[0].replace(/,/g, '.')
	var rp = txt + ',' + comma[comma.length - 1].replace('.', ',')

	return rp
}

function romawi(nomor) {
	var desimal = [1000, 900, 500, 400, 100, 90, 50, 40, 10, 9, 5, 4, 1];
	var romawi = ['M', 'CM', 'D', 'CD', 'C', 'XC', 'L', 'XL', 'X', 'IX', 'V', 'IV', 'I'];

	var hasil = '';

	for (var index = 0; index < desimal.length; index++) {
		while (desimal[index] <= nomor) {
			hasil += romawi[index];
			nomor -= desimal[index];
		}
	}
	return hasil;
}
function groupBy(xs, f) {
	return xs.reduce((r, v, i, a, k = f(v)) => ((r[k] || (r[k] = [])).push(v), r), {});
}
module.exports = function (data) {
	moment.locale('id')
	console.log(data)

	var dataBody = [];
	var sumDataBody = [];
	var sub_header = [];
	var tbl_data = [];
	var group = groupBy(data, (c) => c.kelompok_kendaraan);
	sub_header.push(Object.keys(group, 'kelompok_kendaraan'));
	//grand total
	var tot_kondisi_b = 0;
	var tot_kondisi_rr = 0;
	var tot_kondisi_rb = 0;
	var tot_kondisi_h = 0;
	var tot_nil = 0;
	var tot_nilai_residu = 0
	var tot_akum_sd_thn_lalu = 0
	var tot_nilai_yg_disusutkan = 0
	var tot_peny_sd_bln_lalu = 0
	var tot_peny_bln_ini = 0
	var tot_peny_sd_bln_ini = 0
	var tot_akum_sd_bln_ini = 0
	var tot_nilai_buku_bln_ini = 0
	for (var j = 0; j < sub_header[0].length; j++) {
		var body_len = group[Object.keys(group)[j]].length
		var sum_kondisi_b = 0;
		

		var sum_kondisi_rr = 0;

		var sum_kondisi_rb = 0;

		var sum_kondisi_h = 0;
		var sum_nil = 0;
		var sum_nilai_residu = 0
		var sum_akum_sd_thn_lalu = 0
		var sum_nilai_yg_disusutkan = 0
		var sum_peny_sd_bln_lalu = 0
		var sum_peny_bln_ini = 0
		var sum_peny_sd_bln_ini = 0
		var sum_akum_sd_bln_ini = 0
		var sum_nilai_buku_bln_ini = 0
		var no = j + 1;


		dataBody.push([
			{ text: romawi(no) + '. ', style: 'tSubHeader' },
			{ text: sub_header[0][j].toUpperCase(), bold: true, colSpan: 4, style: 'tSubHeader', fontSize: 4 },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },

		])
		for (var i = 0; i < body_len; i++) {
			var kondisi_b = 0;
			var kondisi_rb = 0;
			var kondisi_rr = 0;
			var kondisi_h = 0;
			var item = group[Object.keys(group)[j]][i];
			sum_kondisi_b = item.kondisi_aset == 'B' ? sum_kondisi_b += 1 : sum_kondisi_b
			kondisi_b = item.kondisi_aset == 'B' ? kondisi_b = 1 : kondisi_b

			sum_kondisi_rr = item.kondisi_aset == 'R' ? sum_kondisi_rr += 1 : sum_kondisi_rr
			kondisi_rr = item.kondisi_aset == 'R' ? kondisi_rr = 1 : kondisi_rr

			sum_kondisi_rb = item.kondisi_aset == 'U' ? sum_kondisi_rb += 1 : sum_kondisi_rb
			kondisi_rb = item.kondisi_aset == 'U' ? kondisi_rb = 1 : kondisi_rb

			sum_kondisi_h = item.kondisi_aset == 'U' ? sum_kondisi_h += 1 : sum_kondisi_h
			kondisi_h = item.kondisi_aset == 'H' ? kondisi_h = 1 : kondisi_h
			sum_nil += parseFloat(item.nilai_perolehan)
			sum_nilai_residu += parseFloat(item.nilai_residu !== null ? item.nilai_residu : '0')
			sum_akum_sd_thn_lalu += item.akum_sd_thn_lalu
			sum_nilai_yg_disusutkan += item.nilai_yg_disusutkan
			sum_peny_sd_bln_lalu += item.peny_sd_bln_lalu
			sum_peny_bln_ini += item.peny_bln_ini
			sum_peny_sd_bln_ini += item.peny_sd_bln_ini
			sum_akum_sd_bln_ini += parseFloat(item.akum_peny_sd_bln_ini)
			sum_nilai_buku_bln_ini += item.nilai_buku_bln_ini


			//grand total
			tot_kondisi_b = item.kondisi_aset == 'B' ? tot_kondisi_b += 1 : tot_kondisi_b
			tot_kondisi_rr = item.kondisi_aset == 'R' ? tot_kondisi_rr += 1 : tot_kondisi_rr
			tot_kondisi_rb = item.kondisi_aset == 'U' ? tot_kondisi_rb += 1 : tot_kondisi_rb
			tot_kondisi_h = item.kondisi_aset == 'U' ? tot_kondisi_h += 1 : tot_kondisi_h
			tot_nil += parseFloat(item.nilai_perolehan)
			tot_nilai_residu += parseFloat(item.nilai_residu !== null ? item.nilai_residu : '0')
			tot_akum_sd_thn_lalu += item.akum_sd_thn_lalu
			tot_nilai_yg_disusutkan += item.nilai_yg_disusutkan
			tot_peny_sd_bln_lalu += item.peny_sd_bln_lalu
			tot_peny_bln_ini += item.peny_bln_ini
			tot_peny_sd_bln_ini += item.peny_sd_bln_ini
			tot_akum_sd_bln_ini += parseFloat(item.akum_peny_sd_bln_ini)
			tot_nilai_buku_bln_ini += item.nilai_buku_bln_ini
			dataBody.push([
				{ text: (i + 1), style: 'tBody' },
				{ text: item.no_reg1, style: 'tBody' },
				{ text: item.no_reg2, style: 'tBody' },
				{ text: item.no_reg3, style: 'tBody' },
				{ text: item.no_reg4, style: 'tBody' },
				{ text: item.nama_aset, style: 'tBody' },
				{ text: item.aset_type, style: 'tBody' },
				{ text: item.aset_merk, style: 'tBody' },
				{ text: item.no_rangka, style: 'tBody' },
				{ text: item.no_mesin, style: 'tBody' },
				{ text: item.no_polisi, style: 'tBody' },
				{ text: item.tgl_pembuatan !== null ? moment(item.tgl_pembuatan).format('DD-MM-YYYY') : { text: '-', alignment: 'center' }, style: 'tBody' },
				{ text: item.tgl_beli !== null ? moment(item.tgl_beli).format('DD-MM-YYYY') : { text: '-', alignment: 'center' }, style: 'tBody' },
				{ text: item.umur_manfaat, style: 'tBody', alignment: 'center' },
				{ text: item.sd_thn_lalu !== null ? { text: item.sd_thn_lalu, alignment: 'center' } : { text: '-', alignment: 'center' }, style: 'tBody' },
				{ text: item.thn_ini !== null ? item.thn_ini : '-', alignment: 'center', style: 'tBody' },
				{ text: item.sd_tahun_ini !== null ? currency(item.sd_tahun_ini) : currency(0), alignment: 'center', style: 'tBody' },
				{ text: item.nilai_perolehan !== null ? currency(item.nilai_perolehan) : currency(0), style: 'tBody' },
				{ text: item.nilai_residu !== null ? currency(item.nilai_residu) : currency(0), style: 'tBody' },
				{ text: currency(item.akum_sd_thn_lalu), style: 'tBody' },
				{ text: '-', style: 'tBody' },
				{ text: '-', style: 'tBody' },
				{ text: item.nilai_yg_disusutkan !== null ? currency(item.nilai_yg_disusutkan) : currency(0), style: 'tBody' },
				{ text: '-', style: 'tBody' },
				{ text: item.peny_sd_bln_lalu !== null ? currency(item.peny_sd_bln_lalu) : currency(0), style: 'tBody' },
				{ text: item.peny_bln_ini !== null ? currency(item.peny_bln_ini) : currency(0), style: 'tBody' },
				{ text: item.peny_sd_bln_ini !== null ? currency(item.peny_sd_bln_ini) : currency(0), style: 'tBody' },
				{ text: item.akum_peny_sd_bln_ini !== null ? currency(item.akum_peny_sd_bln_ini) : currency(0), style: 'tBody' },
				{ text: item.nilai_buku_bln_ini !== null ? currency(item.nilai_buku_bln_ini) : currency(0), style: 'tBody' },
				{ text: kondisi_b, style: 'tBody', alignment: 'center' },
				{ text: kondisi_rr, style: 'tBody', alignment: 'center' },
				{ text: kondisi_rb, style: 'tBody', alignment: 'center' },
				{ text: kondisi_h, style: 'tBody', alignment: 'center' },
				{ text: item.keterangan, style: 'tBody' }
			])
			if (i === 3) {
				break;
			}
		}
		dataBody.push(
			[
				{ text: '0', style: 'tSubHeader' },
				{ text: 'JUMLAH', colSpan: 16, style: 'tSubHeader', fontSize: 4 },
				{ text: '', style: 'tBody' },
				{ text: '', style: 'tBody' },
				{ text: '', style: 'tBody' },
				{ text: '', style: 'tBody' },
				{ text: '', style: 'tBody' },
				{ text: '', style: 'tBody' },
				{ text: '', style: 'tBody' },
				{ text: '', style: 'tBody' },
				{ text: '', style: 'tBody' },
				{ text: '', style: 'tBody' },
				{ text: '', style: 'tBody' },
				{ text: '', style: 'tBody' },
				{ text: '', style: 'tBody' },
				{ text: '', style: 'tBody' },
				{ text: '', style: 'tBody' },
				{ text: currency(sum_nil), style: 'tBody' },
				{ text: currency(sum_nilai_residu), style: 'tBody' },
				{ text: currency(sum_akum_sd_thn_lalu), style: 'tBody' },
				{ text: '-', style: 'tBody', alignment: 'center' },
				{ text: '-', style: 'tBody', alignment: 'center' },
				{ text: currency(sum_nilai_yg_disusutkan), style: 'tBody' },
				{ text: '-', style: 'tBody' },
				{ text: currency(sum_peny_sd_bln_lalu), style: 'tBody' },
				{ text: currency(sum_peny_bln_ini), style: 'tBody' },
				{ text: currency(sum_peny_sd_bln_ini), style: 'tBody' },
				{ text: currency(sum_akum_sd_bln_ini), style: 'tBody' },
				{ text: currency(sum_nilai_buku_bln_ini), style: 'tBody' },
				{ text: sum_kondisi_b, style: 'tBody', alignment: 'center' },
				{ text: sum_kondisi_rr, style: 'tBody', alignment: 'center' },
				{ text: sum_kondisi_rb, style: 'tBody', alignment: 'center' },
				{ text: sum_kondisi_h, style: 'tBody', alignment: 'center' },
				{ text: '-', style: 'tBody', alignment: 'center' },

			]
		)
	}
	return {

		pageSize: 'A4',
		pageOrientation: 'landscape',
		pageMargins: [10, 215, 0, 0],
		header: function (currentPage, pageCount, pageSize) {
			var title = [];
			title.push(
				{
					margin: [6, 18, 0, 20],
					table: {

						widths: [230, 320, 250],

						body: [
							[{
								rowSpan: 3,
								image: 'data:image/png;base64,',
								width: 170,
								height: 40,
								margin: [5, 20, 0, 0]
							},
							{ text: 'FORMULIR', margin: [15, 10, 10, 10], fontSize: 15, bold: true, alignment: 'center' }
								, {

								rowSpan: 4,
								text: 'DAFTAR ASET TETAP',
								alignment: 'center',
								bold: true,
								fontSize: 15,
								margin: [0, 35, 0, 0],
								fillColor: '#969696',
							}],
							['', { text: 'No Dokumen: ', margin: [0, 8, 0, 0], fontSize: 10, border: [false, false, false, false] }, ''],
							['', { text: 'No Revisi       : ', margin: [0, 0, 0, 8], fontSize: 10, border: [false, false, false, false] }, ''],
							[{ text: 'Halaman : ' + currentPage + ' dari ' + pageCount, fontSize: 10, margin: [5, 3, 3, 3] }, { text: 'Tanggal Dikeluarkan :', fontSize: 10, margin: [5, 3, 3, 3] }, ''],

						]
					},
				},
				{
					text: 'DAFTAR ASET TETAP YANG MASIH DIGUNAKAN',
					alignment: 'center',
					bold: true,
					fontSize: 10
				},
				{
					margin: [12, 0, 0, 0],
					text: '\nBPJS KETENAGAKERJAAN RUSUNAWA ' + data[0].nama_rusun + '\nKELOMPOK ASET : KENDARAAN DINAS',
					fontSize: 8,

				},
				{
					margin: [12, 0, 0, 0],
					text: '\nPER ' + moment(data[0].periode_cetak).format('MMMM YYYY').toString().toUpperCase(),
					fontSize: 7
				}
			)
			return title
		},
		content: [
			{
				margin: [-9, 10, 0, 0],
				table: {

					widths: [6, 8, 10, 10, 10, 20, 18, 18, 20, 20, 20, 21, 21, 10, 21, 20, 20, 20, 20, 20, 20, 20, 20, 20, 10, 10, 10, 20, 20, 10, 10, 10, 10, 10, 20],
					headerRows: 3,
					body: [
						[
							{ text: 'NO', bold: true, rowSpan: 2, alignment: 'center', style: 'headerBody', margin: [0, 15, 0, 0] },
							{ text: 'NO REGISTRASI', bold: true, colSpan: 4, alignment: 'center', style: 'headerBody', fontSize: 4 },
							'',
							'',
							'',
							{ text: 'NAMA ASET TETAP', bold: true, rowSpan: 2, alignment: 'center', style: 'headerBody', margin: [0, 10, 0, 0] },

							{ text: 'TYPE', bold: true, rowSpan: 2, alignment: 'center', style: 'headerBody', margin: [0, 10, 0, 0] },
							{ text: 'MERK', bold: true, rowSpan: 2, alignment: 'center', style: 'headerBody', margin: [0, 10, 0, 0] },
							{ text: 'NOMOR', bold: true, colSpan: 3, alignment: 'center', style: 'headerBody', fontSize: 4 },
							'',
							'',
							{ text: 'TAHUN\nPEMBUATAN', bold: true, rowSpan: 2, alignment: 'center', style: 'headerBody', margin: [0, 10, 0, 0] },
							{ text: 'TGL\nBELI', bold: true, rowSpan: 2, alignment: 'center', style: 'headerBody', margin: [0, 10, 0, 0] },
							{ text: 'UM', bold: true, rowSpan: 2, alignment: 'center', style: 'headerBody', margin: [0, 10, 0, 0] },
							{ text: 'SD TH ' + moment(data[0].periode_cetak).add(-1, 'years').format('YYYY'), bold: true, rowSpan: 2, alignment: 'center', style: 'headerBody', margin: [0, 10, 0, 0] },
							{ text: 'TH ' + moment(data[0].periode_cetak).format('YYYY'), bold: true, rowSpan: 2, alignment: 'center', style: 'headerBody', margin: [0, 10, 0, 0] },
							{ text: 'SD TH ' + moment(data[0].periode_cetak).format('YYYY'), bold: true, rowSpan: 2, alignment: 'center', style: 'headerBody', margin: [0, 10, 0, 0] },
							{ text: 'NILAI PEROLEHAN', bold: true, rowSpan: 2, alignment: 'center', style: 'headerBody', margin: [0, 10, 0, 0] },
							{ text: 'NILAI RESIDU', bold: true, rowSpan: 2, alignment: 'center', style: 'headerBody', margin: [0, 10, 0, 0] },
							{ text: 'AKUM PENY SD THN LALU', bold: true, rowSpan: 2, alignment: 'center', style: 'headerBody', margin: [0, 10, 0, 0] },
							{ text: 'PENURUNAN NILAI', bold: true, rowSpan: 2, alignment: 'center', style: 'headerBody', margin: [0, 10, 0, 0] },
							{ text: 'KOREKSI PENYUSUTAN NILAI', bold: true, rowSpan: 2, alignment: 'center', style: 'headerBody', margin: [0, 10, 0, 0] },
							{ text: 'NILAI \nYANG DISUSUTKAN', bold: true, rowSpan: 2, alignment: 'center', style: 'headerBody', margin: [0, 10, 0, 0] },
							{ text: 'SISA UM TH ' + moment(data[0].periode_cetak).format('YYYY'), bold: true, rowSpan: 2, alignment: 'center', style: 'headerBody', margin: [0, 10, 0, 0] },
							{ text: 'BEBAN PENYUSUTAN TH BERJALAN', bold: true, colSpan: 3, alignment: 'center', style: 'headerBody', fontSize: 4 },
							'',
							'',
							{ text: 'AKUM PENY SD BULAN INI', bold: true, rowSpan: 2, alignment: 'center', style: 'headerBody', margin: [0, 10, 0, 0] },
							{ text: 'NILAI BUKU PER\nBULAN LAPORAN', bold: true, rowSpan: 2, alignment: 'center', style: 'headerBody', margin: [0, 10, 0, 0] },
							{ text: 'KONDISI', bold: true, colSpan: 4, alignment: 'center', style: 'headerBody', fontSize: 4 },
							'',
							'',
							'',
							{ text: 'KETERANGAN', bold: true, rowSpan: 2, alignment: 'center', style: 'headerBody', margin: [0, 10, 0, 0] },

						],
						[
							'',
							{ text: 'kode u.k.', style: 'headerBody' },
							{ text: 'kode A. T', style: 'headerBody' },
							{ text: 'bln. Beli', style: 'headerBody' },
							{ text: 'no. urut', style: 'headerBody' },
							'',
							'',
							'',
							{ text: 'RANGKA', bold: true, alignment: 'center', style: 'headerBody' },
							{ text: 'MESIN', bold: true, alignment: 'center', style: 'headerBody' },
							{ text: 'POLISI', bold: true, alignment: 'center', style: 'headerBody' },
							'',
							'',
							'',
							'',
							'',
							'',
							'',
							'',
							'',
							'',
							'',
							'',
							'',
							{ text: 'SD BLN LALU', bold: true, alignment: 'center', style: 'headerBody' },
							{ text: 'BULAN INI', bold: true, alignment: 'center', style: 'headerBody' },
							{ text: 'SD BULAN INI', bold: true, alignment: 'center', style: 'headerBody' },
							'',
							'',
							{ text: 'BAIK', bold: true, alignment: 'center', style: 'headerBody' },
							{ text: 'RUSAK', bold: true, alignment: 'center', style: 'headerBody' },
							{ text: 'USANG', bold: true, alignment: 'center', style: 'headerBody' },
							{ text: 'HILANG', bold: true, alignment: 'center', style: 'headerBody' },
							'',

						],
						[
							{ text: '1', style: 'indexBar' },
							{ text: '2', style: 'indexBar' },
							{ text: '3', style: 'indexBar' },
							{ text: '4', style: 'indexBar' },
							{ text: '5', style: 'indexBar' },
							{ text: '6', style: 'indexBar' },
							{ text: '7', style: 'indexBar' },
							{ text: '8', style: 'indexBar' },
							{ text: '9', style: 'indexBar' },
							{ text: '10', style: 'indexBar' },
							{ text: '11', style: 'indexBar' },
							{ text: '12', style: 'indexBar' },
							{ text: '12/01/1900', style: 'indexBar' },
							{ text: '', style: 'indexBar' },
							{ text: moment(data[0].periode_cetak).add(-1, 'years').format('DD-MM-YYYY'), style: 'indexBar' },
							{ text: moment(data[0].periode_cetak).format('DD-MM-YYYY'), style: 'indexBar' },
							{ text: '', style: 'indexBar' },
							{ text: '14', style: 'indexBar' },
							{ text: '15', style: 'indexBar' },
							{ text: '16', style: 'indexBar' },
							{ text: '17', style: 'indexBar' },
							{ text: '18', style: 'indexBar' },
							{ text: '19', style: 'indexBar' },
							{ text: '', style: 'indexBar' },
							{ text: '20', style: 'indexBar' },
							{ text: '21', style: 'indexBar' },
							{ text: '22=20+21', style: 'indexBar' },
							{ text: '23=22+16', style: 'indexBar' },
							{ text: '24=14-23', style: 'indexBar' },
							{ text: 'Isi dengan tanda angka " 1 "', style: 'indexBar', colSpan: 4 },
							{ text: '', style: 'indexBar' },
							{ text: '', style: 'indexBar' },
							{ text: '', style: 'indexBar' },
							{ text: '25', style: 'indexBar' },

						],
						...dataBody,
						/* [
							{ text: '0', style: 'tSubHeader' },
							{ text: 'JUMLAH TOTAL', colSpan: 16, style: 'tSubHeader' },
							{ text: '', style: 'tBody' },
							{ text: '', style: 'tBody' },
							{ text: '', style: 'tBody' },
							{ text: '', style: 'tBody' },
							{ text: '', style: 'tBody' },
							{ text: '', style: 'tBody' },
							{ text: '', style: 'tBody' },
							{ text: '', style: 'tBody' },
							{ text: '', style: 'tBody' },
							{ text: '', style: 'tBody' },
							{ text: '', style: 'tBody' },
							{ text: '', style: 'tBody' },
							{ text: '', style: 'tBody' },
							{ text: '', style: 'tBody' },
							{ text: '', style: 'tBody' },
							{ text: currency(tot_nil), style: 'tBody' },
							{ text: currency(tot_nilai_residu), style: 'tBody' },
							{ text: currency(tot_akum_sd_thn_lalu), style: 'tBody' },
							{ text: '-', style: 'tBody', alignment: 'center' },
							{ text: '-', style: 'tBody', alignment: 'center' },
							{ text: currency(tot_nilai_yg_disusutkan), style: 'tBody' },
							{ text: '-', style: 'tBody' },
							{ text: currency(tot_peny_sd_bln_lalu), style: 'tBody' },
							{ text: currency(tot_peny_bln_ini), style: 'tBody' },
							{ text: currency(tot_peny_sd_bln_ini), style: 'tBody' },
							{ text: currency(tot_akum_sd_bln_ini), style: 'tBody' },
							{ text: currency(tot_nilai_buku_bln_ini), style: 'tBody' },
							{ text: tot_kondisi_b, style: 'tBody', alignment: 'center' },
							{ text: tot_kondisi_rr, style: 'tBody', alignment: 'center' },
							{ text: tot_kondisi_rb, style: 'tBody', alignment: 'center' },
							{ text: tot_kondisi_h, style: 'tBody', alignment: 'center' },
							{ text: '-', style: 'tBody', alignment: 'center' },

						] */
						[
							{
								margin: [-5, -1, 0, 0],
								table: {
									widths: [6, 8, 10, 10, 10, 20, 18, 18, 20, 20, 20, 20, 21, 10, 21, 21, 20, 20, 20, 20, 20, 20, 20, 20, 10, 10, 10, 20, 20, 10, 10, 10, 10, 10, 20],
									body: [
										[
											{ text: '0', style: 'tSubHeader' },
											{ text: 'JUMLAH TOTAL', colSpan: 16, style: 'tSubHeader', fontSize: 4 },
											{ text: '', style: 'tBody' },
											{ text: '', style: 'tBody' },
											{ text: '', style: 'tBody' },
											{ text: '', style: 'tBody' },
											{ text: '', style: 'tBody' },
											{ text: '', style: 'tBody' },
											{ text: '', style: 'tBody' },
											{ text: '', style: 'tBody' },
											{ text: '', style: 'tBody' },
											{ text: '', style: 'tBody' },
											{ text: '', style: 'tBody' },
											{ text: '', style: 'tBody' },
											{ text: '', style: 'tBody' },
											{ text: '', style: 'tBody' },
											{ text: '', style: 'tBody' },
											{ text: currency(tot_nil), style: 'tBody' },
											{ text: currency(tot_nilai_residu), style: 'tBody' },
											{ text: currency(tot_akum_sd_thn_lalu), style: 'tBody' },
											{ text: '-', style: 'tBody', alignment: 'center' },
											{ text: '-', style: 'tBody', alignment: 'center' },
											{ text: currency(tot_nilai_yg_disusutkan), style: 'tBody' },
											{ text: '-', style: 'tBody' },
											{ text: currency(tot_peny_sd_bln_lalu), style: 'tBody' },
											{ text: currency(tot_peny_bln_ini), style: 'tBody' },
											{ text: currency(tot_peny_sd_bln_ini), style: 'tBody' },
											{ text: currency(tot_akum_sd_bln_ini), style: 'tBody' },
											{ text: currency(tot_nilai_buku_bln_ini), style: 'tBody' },
											{ text: tot_kondisi_b, style: 'tBody', alignment: 'center' },
											{ text: tot_kondisi_rr, style: 'tBody', alignment: 'center' },
											{ text: tot_kondisi_rb, style: 'tBody', alignment: 'center' },
											{ text: tot_kondisi_h, style: 'tBody', alignment: 'center' },
											{ text: '-', style: 'tBody', alignment: 'center' },

										],
										[
											{ text: '', border: [false, false, false, false] },
											{ text: '\n', alignment: 'center', colSpan: 16, border: [false, false, false, false] },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '\n', colSpan: 17, alignment: 'center', border: [false, false, false, false] },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
										],
										[
											{ text: '', border: [false, false, false, false] },
											{ text: '', alignment: 'center', colSpan: 16, border: [false, false, false, false] },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: 'JAKARTA' + ', ' + moment().format('DD MMMM YYYY'), colSpan: 17, alignment: 'center', border: [false, false, false, false] },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
										],
										[
											{ text: '', border: [false, false, false, false] },
											{ text: 'Mengetahui', alignment: 'center', colSpan: 16, border: [false, false, false, false] },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: 'Yang Membuat', colSpan: 17, alignment: 'center', border: [false, false, false, false] },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },

										],
										[
											{ text: '', border: [false, false, false, false] },
											{ text: '\n\n\n\n\n', alignment: 'center', colSpan: 16, border: [false, false, false, false] },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '\n\n\n\n\n', colSpan: 17, alignment: 'center', border: [false, false, false, false] },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
										],
										[
											{ text: '', border: [false, false, false, false] },
											{ text: data[0].assigned_mengetahui + '\n' + data[0].assigned_jabatan_mengetahui, alignment: 'center', colSpan: 16, border: [false, false, false, false] },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: data[0].assigned_membuat + '\n' + data[0].assigned_jabatan_membuat, colSpan: 17, alignment: 'center', border: [false, false, false, false] },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
											{ text: '' },
										],

									]
								},
								style: 'tAssign',
								colSpan: 35,
								unbreakable: true,
								border: [false, false, false, false],
							},
						]

					]
				},
				layout: 'Borders',
			},
			{
				text: '\n'
			},
			{
				text: '\n'
			},
			{
				//margin: [0, 10, 20, 20],
				table: {
					//widths: [10, 30],
					body: [
						[{ text: 'Catatan :', rowSapan: 3 }, '1. Nilai Residu Sedan 25% dari Harga Perolehan'],
						['', '2. Nilai Residu Non Sedan 20% dari Harga Perolehan'],
						['', '3. Nilai Residu Motor 10% dari Harga Perolehan'],
						['', 'Keterangan: disi dengan warna kendaraan'],
					]
				},
				layout: 'noBorders',
				style: 'tNote',
				unbreakable: true,
			},

		],

		styles: {
			indexBar: {
				fontSize: 3,
				bold: true,
				alignment: 'center'
			},
			header: {
				fontSize: 15,
				alignment: 'justify',
			},
			subHeader: {
				fontSize: 14,
				bold: true,
			},
			headerBody: {
				fontSize: 3,
				fillColor: '#969696',
				alignment: 'center'
			},
			textBody: {
				fontSize: 2.5,
				bold: true,
				alignment: 'center',
			},
			tSubHeader: {
				fontSize: 3,
				bold: true,
				alignment: 'center'
			},
			tBody: {
				fontSize: 4,
			},
			tSpacer: {
				fillColor: '#969696',
			},
			tAssign: {
				fontSize: 10,
				alignment: 'center'
			},
			tNote: {
				fontSize: 8,
				bold: true,
			}
		},
		defaultStyle: {
			// alignment: 'justify'
		}

	}
}
