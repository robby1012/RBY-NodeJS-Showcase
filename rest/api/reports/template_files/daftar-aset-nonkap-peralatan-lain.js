const moment = require('moment')
const { isNumber } = require("validate.js");

function currency(curr) {
	var data;
	if (curr === null) {
		curr = 0
	}
	if (!isNumber(curr)) {
		data = (parseFloat(curr)).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
	} else if (isNumber(curr)) {
		data = (curr).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
	}

	//console.log(data.toString())
	var str = data.toString();

	var comma = str.split('.');
	var txt = comma[0].replace(/,/g, '.')
	var rp = txt + ',' + comma[comma.length - 1].replace('.', ',')

	return rp
}

function romawi(nomor) {
	var desimal = [1000, 900, 500, 400, 100, 90, 50, 40, 10, 9, 5, 4, 1];
	var romawi = ['M', 'CM', 'D', 'CD', 'C', 'XC', 'L', 'XL', 'X', 'IX', 'V', 'IV', 'I'];

	var hasil = '';

	for (var index = 0; index < desimal.length; index++) {
		while (desimal[index] <= nomor) {
			hasil += romawi[index];
			nomor -= desimal[index];
		}
	}
	return hasil;
}
function groupBy(xs, f) {
	return xs.reduce((r, v, i, a, k = f(v)) => ((r[k] || (r[k] = [])).push(v), r), {});
}

module.exports = function (data) {
	console.log(data)
	moment.locale('id');

	if (data.length == 0) {
		var data = [
			{
				periode_cetak: '2020-11-10',
				nama_rusun: 'JABABEKA CIKARANG 1',
			}
		]
	}

	var dataBody = [];
	var sumDataBody = [];
	var sub_header = [];
	var tbl_data = [];
	var group = groupBy(data, (c) => c.nama_kategori);
	sub_header.push(Object.keys(group, 'nama_kategori'));
	//grand total
	var tot_kondisi_b = 0;
	var tot_kondisi_rr = 0;
	var tot_kondisi_rb = 0;
	var tot_kondisi_h = 0;
	var tot_nil = 0;
	var tot_nilai_perolehan = 0

	for (var j = 0; j < sub_header[0].length; j++) {
		var body_len = group[Object.keys(group)[j]].length
		var sum_kondisi_b = 0;

		var sum_kondisi_rr = 0;

		var sum_kondisi_rb = 0;

		var sum_kondisi_h = 0;

		var sum_nil = 0;

		var no = j + 1;

		dataBody.push([
			{ text: romawi(no) + '. ', style: 'tSubHeader' },
			{ text: sub_header[0][j].toUpperCase(), bold: true, colSpan: 4, style: 'tSubHeader' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
		])

		for (var i = 0; i < body_len; i++) {
			var item = group[Object.keys(group)[j]][i];
			var kondisi_b = 0;
			var kondisi_rr = 0;
			var kondisi_rb = 0;
			var kondisi_h = 0;


			sum_kondisi_b = item.kondisi_aset == 'B' ? sum_kondisi_b += 1 : sum_kondisi_b
			kondisi_b = item.kondisi_aset == 'B' ? c = 1 : kondisi_b

			sum_kondisi_rr = item.kondisi_aset == 'R' ? sum_kondisi_rr += 1 : sum_kondisi_rr
			kondisi_rr = item.kondisi_aset == 'R' ? kondisi_rr = 1 : kondisi_rr

			sum_kondisi_rb = item.kondisi_aset == 'U' ? sum_kondisi_rb += 1 : sum_kondisi_rb
			kondisi_rb = item.kondisi_aset == 'U' ? kondisi_rb = 1 : kondisi_rb

			sum_kondisi_h = item.kondisi_aset == 'U' ? sum_kondisi_h += 1 : sum_kondisi_h
			kondisi_h = item.kondisi_aset == 'H' ? kondisi_h = 1 : kondisi_h

			sum_nil += parseFloat(item.nilai_perolehan)

			//grand total
			tot_kondisi_b = item.kondisi_aset == 'B' ? tot_kondisi_b += 1 : tot_kondisi_b
			tot_kondisi_rr = item.kondisi_aset == 'R' ? tot_kondisi_rr += 1 : tot_kondisi_rr
			tot_kondisi_rb = item.kondisi_aset == 'U' ? tot_kondisi_rb += 1 : tot_kondisi_rb
			tot_kondisi_h = item.kondisi_aset == 'U' ? tot_kondisi_h += 1 : tot_kondisi_h
			tot_nil += parseFloat(item.nilai_perolehan)

			dataBody.push([
				{ text: (i + 1), style: 'tNumber' },
				{ text: item.no_reg1, style: 'tBody' },
				{ text: item.no_reg2, style: 'tBody' },
				{ text: item.no_reg3, style: 'tBody' },
				{ text: item.no_reg4, style: 'tBody' },
				{ text: item.nama_aset, style: 'tBody' },
				{ text: item.aset_type, style: 'tBody' },
				{ text: item.tgl_beli !== null ? moment(item.tgl_beli).format('DD-MM-YYYY') : '-', style: 'tBody' },
				{ text: currency(item.nilai_perolehan), style: 'tBody' },
				{ text: kondisi_b, style: 'tBody', alignment: 'center' },
				{ text: kondisi_rr, style: 'tBody', alignment: 'center' },
				{ text: kondisi_rb, style: 'tBody', alignment: 'center' },
				{ text: kondisi_h, style: 'tBody', alignment: 'center' },
				{ text: item.keterangan, style: 'tBody' }
			])
		}

		dataBody.push([
			{ text: '0', style: 'tSubHeader' },
			{ text: 'JUMLAH', colSpan: 7, style: 'tSubHeader' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: '', style: 'tBody' },
			{ text: currency(sum_nil), style: 'tBody' },
			{ text: sum_kondisi_b, style: 'tBody', alignment: 'center' },
			{ text: sum_kondisi_rr, style: 'tBody', alignment: 'center' },
			{ text: sum_kondisi_rb, style: 'tBody', alignment: 'center' },
			{ text: sum_kondisi_h, style: 'tBody', alignment: 'center' },
			{ text: '-', style: 'tBody', alignment: 'center' },
		])
	}
	console.log(dataBody)

	return {
		pageSize: 'A4',
		pageOrientation: 'potrait',
		pageMargins: [10, 230, 0, 0],
		header: function (currentPage, pageCount, pageSize) {
			var title = [];
			title.push(
				{
					margin: [6, 18, 0, 20],
					table: {

						widths: [185, 200, 170],

						body: [
							[{
								rowSpan: 3,
								image: 'data:image/png;base64,',
								width: 170,
								height: 40,
								margin: [5, 20, 0, 0]
							},
							{ text: 'FORMULIR', margin: [15, 10, 10, 10], fontSize: 15, bold: true, alignment: 'center' }
								, {

								rowSpan: 4,
								text: 'DAFTAR ASET NON KAPITALISASI',
								alignment: 'center',
								bold: true,
								fontSize: 15,
								margin: [0, 35, 0, 0],
								fillColor: '#969696',
							}],
							['', { text: 'No Dokumen: ', margin: [0, 8, 0, 0], fontSize: 10, border: [false, false, false, false] }, ''],
							['', { text: 'No Revisi       : ', margin: [0, 0, 0, 8], fontSize: 10, border: [false, false, false, false] }, ''],
							[{ text: 'Halaman : ' + currentPage + ' dari ' + pageCount, fontSize: 10, margin: [5, 3, 3, 3] }, { text: 'Tanggal Dikeluarkan :', fontSize: 10, margin: [5, 3, 3, 3] }, ''],

						]
					},
				},
				{
					text: 'DAFTAR  ASET YANG MASIH DIGUNAKAN NON KAPITALISASI',
					alignment: 'center',

					bold: true,
					fontSize: 12
				},
			)
			return title
		},


		content: [
			{
				text: [
					'BPJS KETENAGAKERJAAN ' + data[0].nama_rusun + '\nKELOMPOK ASET: PERALATAN LAIN\n\nPER ' + moment(data[0].periode_cetak).format('MMMM YYYY').toString().toUpperCase(),
				],
				fontSize: 12,
				bold: false,
				margin: [0, -50, 0, 0]
			},
			{
				margin: [0, 10, 0, 0],
				table: {
					widths: [15, 10, 10, 10, 13, 80, 50, 35, 50, 30, 30, 30, 30, 55],
					headerRows: 3,
					heights: function (row) {
						if (row == 3) {
							return row * 0;
						}
					},
					body: [
						[{
							rowSpan: 2,
							text: '\nNO',
							style: 'headerBody',
							fillColor: '#969696'
						},
						{
							rowSpan: 2,
							colSpan: 4,
							text: '\nNO. REGISTRASI',
							style: 'headerBody',
							fillColor: '#969696'
						},
							'', '', '',
						{
							rowSpan: 2,
							text: '\nNAMA ASET TETAP',
							style: 'headerBody',
							fillColor: '#969696'
						},
						{
							rowSpan: 2,
							text: '\nMERK/TYPE',
							style: 'headerBody',
							fillColor: '#969696'
						},
						{
							rowSpan: 2,
							text: '\nTGL BELI',
							style: 'headerBody',
							fillColor: '#969696'
						},
						{
							rowSpan: 2,
							text: '\nNILAI PEROLEHAN',
							style: 'headerBody',
							fillColor: '#969696'
						},
						{
							colSpan: 4,
							text: 'KONDISI',
							style: 'headerBody',
							fillColor: '#969696'
						}, '', '', '',
						{
							rowSpan: 2,
							text: '\nKET',
							style: 'headerBody',
							fillColor: '#969696'
						}
						],

						['', '', '', '', '', '', '', '', '',
							{
								text: 'BAIK',
								style: 'headerBody',
								fillColor: '#969696'
							},
							{
								text: 'RUSAK',
								style: 'headerBody',
								fillColor: '#969696'
							},
							{
								text: 'USANG',
								style: 'headerBody',
								fillColor: '#969696'
							},
							{
								text: 'HILANG',
								style: 'headerBody',
								fillColor: '#969696'
							}, ''
						],

						[{ text: '1', style: 'indexBar' }, { text: '2', style: 'indexBar' }, { text: '3', style: 'indexBar' }, { text: '4', style: 'indexBar' }, { text: '5', style: 'indexBar' }, { text: '6', style: 'indexBar' }, { text: '7', style: 'indexBar' }, { text: '8', style: 'indexBar' }, { text: '9', style: 'indexBar' },
						{
							colSpan: 4,
							text: 'Isi dengan tanda angka " 1 "',
							style: 'indexBar'
						},
							'', '', '', { text: '10', style: 'indexBar' }
						],
						...dataBody,

						[
							{ text: '0', style: 'tSubHeader' },
							{ text: 'JUMLAH TOTAL', colSpan: 7, style: 'tSubHeader' },
							{ text: '', style: 'tBody' },
							{ text: '', style: 'tBody' },
							{ text: '', style: 'tBody' },
							{ text: '', style: 'tBody' },
							{ text: '', style: 'tBody' },
							{ text: '', style: 'tBody' },
							{ text: currency(tot_nil), style: 'tBody' },
							{ text: tot_kondisi_b, style: 'tBody', alignment: 'center' },
							{ text: tot_kondisi_rr, style: 'tBody', alignment: 'center' },
							{ text: tot_kondisi_rb, style: 'tBody', alignment: 'center' },
							{ text: tot_kondisi_h, style: 'tBody', alignment: 'center' },
							{ text: '-', style: 'tBody', alignment: 'center' },
						],

					],
				},
				layout: 'Borders'
			},

			{
				margin: [0, 20, 0, 0],
				alignment: 'center',
				fontSize: 11,
				table: {
					widths: [275, 275],
					//heights:[100,100],
					body: [
						['', { text: 'JAKARTA' + ', ' + moment().format('DD MMMM YYYY') }],
						['Mengetahui,', 'Yang Membuat,'],
						['', ''],
						['', ''],
						['', ''],
						['', ''],
						['', ''],
						['', ''],
						['', ''],
						['', ''],
						['', ''],
						['', ''],
						['', ''],

						[data[0].assigned_mengetahui, data[0].assigned_membuat],
						[data[0].assigned_jabatan_mengetahui, data[0].assigned_jabatan_membuat],
					],

				},
				style: 'tAssign',
				layout: 'noBorders',
				unbreakable: true,
			},
			{
				text: '\n'
			},
		],

		styles: {
			indexBar: {
				fontSize: 4,
				bold: true,
				alignment: 'center'
			},
			header: {
				fontSize: 15,
				alignment: 'justify',
			},
			subHeader: {
				fontSize: 14,
				bold: true,
			},
			headerBody: {
				fontSize: 4.2,
				fillColor: '#969696',
				alignment: 'center'
			},
			textBody: {
				fontSize: 4,
				bold: true,
				alignment: 'center',
			},
			noborder: {
				border: [false, false, false, false],
			},
			tSubHeader: {
				fontSize: 5,
				bold: true,
				alignment: 'center'
			},
			tBody: {
				fontSize: 4,
			},
			tNumber: {
				fontSize: 4,
				alignment: 'center'
			},
			tSpacer: {
				fillColor: '#969696',
			},
			tAssign: {
				fontSize: 10,
				bold: true,
				alignment: 'center'
			},
			tNote: {
				fontSize: 8,
				bold: true,
			}
		}
	}
}